...
struct _Params
{
	...
	double ROI_col2;
	double AreaUpLimt;
	double AreaDownLimit;
	double Reserved;//保留参数，用作其他用，比如面积
};
...
public:
	...
	void set_wnd(HWND hWnd){ if (!WindowHandle)  WindowHandle = (Hlong)hWnd; }
	void set_wnd1(HWND hWnd){ if (!WindowHandle1)  WindowHandle1 = (Hlong)hWnd; }
	...
	const HTuple & get_disp_hd() { return m_dis_hd; } 
	const HTuple & get_disp_hd1(){ return m_dis_hd1;}
private:
