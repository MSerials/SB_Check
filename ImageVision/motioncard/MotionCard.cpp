...
int Motion::SysToOrigin()
{
	...
	int res = BackToOrigin(FIRST_MOTOR, 100,g.ini.m_FstMtr.InitSpeed, g.ini.m_FstMtr.Tacc, g.ini.m_FstMtr.compansate);
	if (res) return res;
	res = BackToOrigin(SECOND_MOTOR, 100,g.ini.m_SndMtr.InitSpeed,g.ini.m_SndMtr.Tacc, g.ini.m_SndMtr.compansate);
	if (res) return res;
#endif
	return 0;
}
...
int Motion::BackToOrigin(short Axis, int initspeed, int speed, double Tacc, int compansate)
{
	...
	int ORGIN_SENSOR = MOTOR2_ORIGIN;
#if 0
	...
	if (read_input_state(UP_LIMIT))			//感应器接反可能有bug//
	{
		...
		{
			d1000_start_tv_move(Axis, -1*initspeed/5, -1*speed/5, Tacc);
