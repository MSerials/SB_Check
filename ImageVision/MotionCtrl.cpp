...
void CMotionCtrl::OnBnClickedButtonComfirm()
{
	...
	GetDlgItem(IDC_EDIT_BACKSPEED)->GetWindowText(str);
	g.ini.m_FstMtr.BackSpeed = _ttoi(str);
...
BOOL CMotionCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_LBUTTONDOWN)
	{
		if (pMsg->hwnd == GetDlgItem(IDC_BUTTON_FORWARD)->m_hWnd)
		{
			...
			g.mc.MoveForward(FIRST_MOTOR,g.ini.m_FstMtr.MaxSpeed/2);
		}
		if (pMsg->hwnd == GetDlgItem(IDC_BUTTON_BACKWORD)->m_hWnd) //后退
		{
			...
			g.mc.MoveBackward(FIRST_MOTOR, -1*g.ini.m_FstMtr.MaxSpeed/2);
		}
		...
		if (pMsg->hwnd == GetDlgItem(IDC_BUTTON_FORWARD2)->m_hWnd)
		{
			...
			g.mc.MoveForward(SECOND_MOTOR, g.ini.m_SndMtr.MaxSpeed/2);
		}
		if (pMsg->hwnd == GetDlgItem(IDC_BUTTON_BACKWORD2)->m_hWnd) //后退
		{
			...
			g.mc.MoveBackward(SECOND_MOTOR,-1*g.ini.m_SndMtr.MaxSpeed/2);
		}
	}
...
BOOL CMotionCtrl::OnInitDialog()
{
	...
	str.Format(_T("%d"), g.ini.m_FstMtr.BackSpeed);
	GetDlgItem(IDC_EDIT_BACKSPEED)->SetWindowText(str);
	str.Format(_T("%d"), g.ini.m_SndMtr.BackSpeed);
	GetDlgItem(IDC_EDIT_BACKSPEED2)->SetWindowText(str);
	...
	//通知动作函数暂停
	ProcedureState = g.evt_procedure.EventState();
...
void CMotionCtrl::OnClose()
{
	// TODO:  在此添加消息处理程序代码和/或调用默认值
	KillTimer(1);
	if (ProcedureState) g.evt_procedure.SetEvent();
	CDialogEx::OnClose();
}
...
void CMotionCtrl::OnBnClickedButtonComfirm2()
{
	...
	GetDlgItem(IDC_EDIT_BACKSPEED2)->GetWindowText(str);
	g.ini.m_SndMtr.BackSpeed = _ttoi(str);
	g.ini.SaveParaFile(PARA_IO);
}
...
UINT CMotionCtrl::HomeThread(LPVOID lp)
{
	...
	p->isHomeThread = true;
	g.mc.BackToOrigin(FIRST_MOTOR, 100,g.ini.m_FstMtr.InitSpeed,  g.ini.m_FstMtr.Tacc, g.ini.m_FstMtr.compansate);
...
UINT CMotionCtrl::HomeThread1(LPVOID lp)
{
	...
	g.mc.BackToOrigin(SECOND_MOTOR, 100,g.ini.m_SndMtr.InitSpeed,g.ini.m_SndMtr.Tacc, g.ini.m_SndMtr.compansate);
