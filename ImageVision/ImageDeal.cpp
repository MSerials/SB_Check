...
bool CImageDeal::Front_Train(const std::vector<std::string>& imgpath, const HTuple& WinHandle, const _Params &params)
{
	...
	try
	{
...
#ifdef _HALCON
		g.show_image.Dis_Obj(TopoImage);
#endif
		...
		Halcon::erosion_rectangle1(TopRegionFillUp, &TopRegionROI, 12, 50);
		write_top_params(g.prjini.m_Prj_Name);
	}
	catch (HException &HDevExpDefaultException)
	{
		set_check("~give_error");
...
int CImageDeal::Front_Deal(const Hobject src, const HTuple& WinHandle, const _Params &params)//image roi,result
{
	...
	try{
		...
		if (0 != ((TopScore.Num()) > params.MatchScore))
		{
			...
			//屏蔽规定的检测区域
			Halcon::Hobject NoNeedDetect, ErrorRegion;
			Halcon::complement(TopNoDetect, &NoNeedDetect);
			Halcon::reduce_domain(TopConnectedRegions, NoNeedDetect, &ErrorRegion);
			Halcon::select_shape(ErrorRegion, &TopRegionsError, "area", "and", 20, 1000000);
	...
	catch (HException &HDevExpDefaultException)
	{
		...
		Dis_Obj(src, WinHandle);
		Halcon::set_color(WinHandle, "red");
		Halcon::write_string(WinHandle, "图像处理错误");
		return -1;
	}
...
bool CImageDeal::Back_Train(const std::vector<std::string>& imgpath, const HTuple& WinHandle, const _Params &params)
{
	...
	try
	{
		...
		g.show_image.Dis_Obj1(BottomoImage);
		...
		erosion_rectangle1(BottomRegionFillUp, &BottomRegionROI, 12, 50);
		write_bottom_params(g.prjini.m_Prj_Name);
	}
	catch (HException &HDevExpDefaultException)
	{
		set_check("~give_error");
...
int CImageDeal::Back_Deal(const Hobject src, const HTuple& WinHandle, const _Params &params)//image roi,result
{
	...
	try
	{
		...
		if (0 != ((BottomScore.Num()) > params.MatchScore))
		{
			...
			//屏蔽检测区域
			Halcon::Hobject NoNeedDetect, ErrorRegion;
			Halcon::complement(BottomNoDetect, &NoNeedDetect);
			Halcon::reduce_domain(BottomConnectedRegions, NoNeedDetect, &ErrorRegion);
			Halcon::select_shape(ErrorRegion, &BottomRegionsError, "area", "and", 20, 1000000);

			Halcon::count_obj(BottomRegionsError, &BottomNumError);
			Halcon::clear_window(WinHandle);
			Halcon::disp_obj(BottomImageTrans, WinHandle);
			Halcon::set_color(WinHandle, "red");
			Halcon::disp_obj(BottomRegionsError, WinHandle);
			Halcon::set_tposition(WinHandle, 20, 20);
			if (0 != (BottomNumError == 0))
			{
				Halcon::set_color(WinHandle, "green");
				Halcon::write_string(WinHandle, "Product is OK");
				return 1;
...
void CImageDeal::Dis_Obj(const Hobject& hobject, const HTuple& WinHandle)
{
	//	Lock.lock();
	try{
		Hlong Width, Height;
		Halcon::get_image_pointer1(hobject, NULL, NULL, &Width, &Height);
		Halcon::set_part(WinHandle, NULL, NULL, Height, Width);
		Halcon::disp_obj(hobject, WinHandle);
	}
...
Halcon::HTuple CImageDeal::CheckHole(const Hobject src, const HTuple& WinHandle, const _Params &params)
{
	try{
		set_check("give_error");
		Halcon::Hobject ROI, ImageReduced, Regions, ConnectedRegions, SelectedRegions, RegionTrans, RegionUnion;
		Halcon::HTuple Area, Row, Column;
		Halcon::gen_rectangle1(&ROI, params.ROI_row1, params.ROI_col1, params.ROI_row2, params.ROI_col2);
		Halcon::reduce_domain(src, ROI, &ImageReduced);
		Halcon::threshold(ImageReduced, &Regions, params.threshold_get_prod, 255);
		Halcon::connection(Regions, &ConnectedRegions);
		Halcon::select_shape(ConnectedRegions, &SelectedRegions, "area", "and", params.AreaDownLimit, params.AreaUpLimt);
		Halcon::shape_trans(SelectedRegions, &RegionTrans, "convex");
		Halcon::union1(RegionTrans, &RegionUnion);
		Halcon::area_center(RegionUnion, &Area, &Row, &Column);	
		Halcon::set_color(WinHandle, "green");
		Halcon::set_draw(WinHandle, "fill");
		Halcon::disp_obj(RegionUnion, WinHandle);
		Halcon::disp_cross(WinHandle, Row,Column,30,0);
		return Area;
	}
	catch (HException& except)
	{
