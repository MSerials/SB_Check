...
enum
{
	SELECTPRJ,SELECTPRJBYNAME,COMMAND
};
...
#define WMMSG_SOCKET_COMMAND (WM_USER+1004)

#define BOTTOMSHAPE L"BOTTOM.shm"
#define TOPSHAPE L"TOP.shm"
...
enum RECTANGLE{
	NO_RECT = 0, TROI_RECT, TCHAR_RECT, BROI_RECT, BCHAR_RECT,TDROI_RECT1,TDROI_RECT2,BDROI_RECT1,BDROI_RECT2, TNROI_RECT, BNROI_RECT
};
...
#define SYSERROR				(1<<16)

#define SENSOR_PRESS			1
#define SENSOR_CYL				2

#define SYSSENSORPRESSURE		(SENSOR_PRESS<<17)
#define SYSSENSORCYL			(SENSOR_CYL<<17)
...
public:
		...
		
	{
		...
		Controller.SysState |= SYSLOADDATAERROR;
		Controller.SysState |= SYSNEEDORIGIN;
		Controller.SysState |= TOPCAMERROR;
		Controller.SysState |= BOTTOMERROR;
//		Controller.SysState |= SYSTOPTRAIN;
//		Controller.SysState |= SYSBOTTOMTRAIN;
		FrontDetectNo = 0;
		BackDetectNo = 0;
		frontNo = 0;
		backNo = 0;
		...
		MainCtrl = NULL;
		CurrentPrj = 0;
		msg_socket = L"SawBar_Check,0,TOTAL,0,OK,0,NG,0,NG,NOT_RUN\n";
...
public:
	...
	{
		CString str;
		if (SYSEMGENCY == (SYSEMGENCY&code)) str += L"急停按钮按下-";
		if (SYSSTOP == (SYSSTOP&code)) str += L"系统停止-";
		if (SYSRUNNING == (SYSRUNNING&code)) str += L"系统运行中-";
		if (SYSLOADDATAERROR == (SYSLOADDATAERROR&code)) str += L"载入数据错误提取中-";
		...
		if (SYSERROR == (SYSERROR&code)) str += L"运行状态不对,不能没提起气缸运动-";
		return str;
	}
	//决定是哪个橡皮筋使用，画哪个对话框
	...
	//开机第一次运行标志，去除第一次显示的bug用的
	int FirstTimeRunSign;
	int CurrentPrj;//当前项目号码
	CString CurrentPrjName;//当前导板项目名称
